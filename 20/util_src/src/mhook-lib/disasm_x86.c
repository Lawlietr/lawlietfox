...
#define VIRTUAL_ADDRESS ((U64)(UINT_PTR)Instruction->Address + Instruction->VirtualAddressDelta)
...
{ \
	...
	switch ( (int)INS_ARCH_TYPE(Instruction) ) \
	{ \
	...
	{ \
		...
		{ \
			...
			case SEG_ES: \
				...
				Operand->TargetAddress = (U64)(UINT_PTR)X86Instruction->Displacement; \
			...
			case SEG_GS: \
				assert(!X86Instruction->HasSelector); \
				Operand->TargetAddress = (U64)(UINT_PTR)GetAbsoluteAddressFromSegment((BYTE)X86Instruction->Segment, (DWORD)X86Instruction->Displacement); \
				break; \
			default: \
...
{ \
	if (!X86Instruction->HasSegmentOverridePrefix && ( (int)reg == (int)REG_EBP || (int)reg == (int)REG_ESP) ) \
	{ \
		assert(!X86Instruction->HasSelector); \
...
HasSpecialExtension:
	...
	if (Instruction->Groups & ITYPE_EXEC)
	{
		...
		if (!Instruction->AnomalyOccurred &&
			Operand1->TargetAddress >= (U64)(UINT_PTR)Instruction->Address &&
			Operand1->TargetAddress < (U64)(UINT_PTR)Instruction->Address + Instruction->Length)
		{
		...
		switch (Instruction->Type)
		{
			...
			case ITYPE_BRANCHCC:
				...
				if (Operand1->TargetAddress)
				{
					...
					Instruction->CodeBranch.Addresses[0] = Operand1->TargetAddress;
					Instruction->CodeBranch.Addresses[1] = (U64)(UINT_PTR)Instruction->Address + Instruction->Length;
					Instruction->CodeBranch.Operand = Operand1;
				}
				break;
	...
	for (OperandIndex = 0; OperandIndex < Instruction->OperandCount; OperandIndex++)
	{
		...
		switch (AddressMode)
		{
			...
			case AMODE_J: // IP-relative jump offset
				...
				Operand->TargetAddress = ApplyDisplacement((U64)(UINT_PTR)Address, Instruction);
				X86Instruction->Relative = TRUE; 
				X86_SET_ADDR();
				SANITY_CHECK_SEGMENT_OVERRIDE();
				X86Instruction->HasSegmentOverridePrefix = FALSE;
			...
			case AMODE_PR: // modrm.rm = mmx register and modrm.mod = 11
				...
				{
					if (!SuppressErrors) printf("[0x%08I64X] ERROR: AMODE_PR illegal %d in 16-bit mode (\"%s\")\n", VIRTUAL_ADDRESS, rex_modrm.rm, X86Instruction->Opcode.Mnemonic);
					goto abort;
				}
				if (!Decode) continue;
			...
			case AMODE_VR: // modrm.rm = xmm register and modrm.mod = 11
				...
				{
					if (!SuppressErrors) printf("[0x%08I64X] ERROR: AMODE_VR illegal %d in 16-bit mode (\"%s\")\n", VIRTUAL_ADDRESS, rex_modrm.rm, X86Instruction->Opcode.Mnemonic);
					goto abort;
				}
				if (!Decode) continue;
...
INTERNAL U8 *SetModRM32(INSTRUCTION *Instruction, U8 *Address, INSTRUCTION_OPERAND *Operand, U32 OperandIndex, BOOL SuppressErrors)
{
	...
	else if (modrm.mod == 0 && modrm.rm == 5)
	{
		...
		if (IS_AMD64())
		{
			...
			Operand->TargetAddress = ApplyDisplacement((U64)(UINT_PTR)Address + ImmediateSize, Instruction);
		}
